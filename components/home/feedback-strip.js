...
export default function FeedbackStrip({ feedbackCards, archive }) {
  return (
    <section id="feedback" className="space-y-5">
      <div className="flex items-end justify-between gap-4">
        ...
        <div id="archive" className="hidden rounded-full border border-[#E8DCF4] bg-white/70 px-4 py-2 text-sm font-semibold text-[#6F6688] md:block">收藏今日反响</div>
      </div>
      ...
      <div className="rounded-[30px] border border-white/80 bg-white/72 p-5 shadow-[0_18px_34px_rgba(86,71,118,0.08)]">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            {archive.map((item) => (
              <a key={item.id} href={item.href} className="rounded-full border border-[#E6DDF6] bg-[#FCFAFF] px-4 py-2 text-sm font-semibold text-[#5F5878] transition hover:-translate-y-0.5 hover:shadow-md">
                {item.date}
              </a>
            ))}
          </div>

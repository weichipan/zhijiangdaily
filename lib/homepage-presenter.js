const MEMBER_PRESETS = [
  {
    ...
    reportTitle: "综艺节奏",
    accent: "pink",
    imageSrc: "/images/group-photo-1.jpg",
  ...
  },
  {
    ...
    feedback: "适合整理情绪类反馈和长评论，做成更柔和的日报段落。",
  },
  {
    id: "bella",
  ...
  },
  {
    ...
    fallbackStatus: "当前首页先保留占位，后续接入独立素材。",
...
function parseLiveMetric(metricValue) {
  if (!metricValue) return { liveCount: "0", totalCount: "0" };
...
export function buildHomepageView({ latestIssue, history, todayLabel, todayDateCard }) {
  ...
  const headlines = MEMBER_PRESETS.slice(0, 3).map((member, index) => {
  ...
  return {
    ...
    latestIssue: {
      ...
      summary: cleanValue(
        latestIssue?.summary,
        "今天的首页先以视觉化日报方式呈现重点版块，后续会继续接入更完整的直播与反馈数据。",
      ),
    },
    recentReports,
    headlines,

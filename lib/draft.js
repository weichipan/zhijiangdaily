...
function buildScheduleHighlights(item = {}) {
  ...
  if (item.rawPayload?.source) {
    parts.push(`来源：${item.rawPayload.source}`);
  }

  if (item.roomId) {
    parts.push(`房间号 ${item.roomId}`);
  }
      ...
      text:
        scheduleItems.length > 0
          ...
          : activeLives.length > 0
            ? `今天共发现 ${activeLives.length} 个成员直播间处于开播状态，系统已经根据直播间状态生成了一版草稿，等待人工审阅。`
